...
bool update_count(egrs_lab7::update_count::Request &req, egrs_lab7::update_count::Response &res);
...
std::string stateToString(RobotState s)
{
  switch (s)
  {
	...
	case WAITIN:		return "Waiting_for_Button";
	case SENT_HOME: 	return "Sent_Home";
	//case TIMEOUT:		return "In_Timeout";
...
bool update_count(egrs_lab7::update_count::Request  &req, egrs_lab7::update_count::Response &res)
{
  ...
  place_count = req.new_count;

  //updates the message variable with the new number
  LED_update.data=place_count;
...
bool go_home(egrs_lab7::go_home::Request &req,
             egrs_lab7::go_home::Response &res)
{
	if(!(state == SENT_HOME))
	{
  res.old_job = state;
  ...
  ROS_WARN("GO HOME service called");
	}
 
  return true;
}

bool return_to_work(egrs_lab7::return_to_work::Request &req,
                    egrs_lab7::return_to_work::Response &res)
{
  res.old_job = saved_state;

  state = normalizeResumeState(saved_state);

  goal_sent = false;
  goal_reached = false;

  ROS_WARN("RETURN TO WORK service called");
...
int main(int argc, char **argv)
{
  ...
  ros::Publisher status_pub = nh.advertise<egrs_lab7::turtlebot_status>("TurtleBot_Status", 1);
  ...
  // Services
  ros::ServiceServer count_service = nh.advertiseService("update_count", update_count); 
  ros::ServiceServer go_home_service = nh.advertiseService("go_home", go_home);

ros::ServiceServer return_service = nh.advertiseService("return_to_work", return_to_work);
  ...
  while (ros::ok())
  {
	...
	egrs_lab7::turtlebot_status status_msg;

status_msg.current_job = stateToString(state);
status_msg.place_count = place_count;
status_msg.battery = battery_low ? LOW_BATTERY_VOLTAGE : HIGH_BATTERY_VOLTAGE;

...
{
  robot_pos = msg->feedback.base_position.pose.position;
}
...
void ParamApply(int x, bool slow_mode)
{
	Params p = param_list[x];

if (slow_mode)
{
  p.max_vel_x = std::min(p.max_vel_x, 0.13);
  p.max_rot_vel = std::min(p.max_rot_vel, 1.0);
}
...
int main(int argc, char **argv)
{
  ...
  ros::Subscriber feedback_sub = n.subscribe("/move_base/feedback", 10, RobotCallback);
  ros::Subscriber human_sub = n.subscribe("human", 10, humanCallback);

  ros::Publisher motor_pub = n.advertise<std_msgs::Bool>("/motor_power", 10);
...
// crosswalk bounds
bool in_crosswalk =(robot_pos.x > X1 && robot_pos.x < X2 &&robot_pos.y < Y1 && robot_pos.y > Y2);

std_msgs::Bool motor_msg;

if (human_active && dist < 0.5)
{
  cancelMoveBaseGoal();

  motor_msg.data = false;   // MOTORS OFF
  motor_pub.publish(motor_msg);

  goal_sent = false;

  ROS_WARN("HUMAN DETECTED - MOTORS OFF");
...
{
  slow_mode = true;
  ROS_INFO("In crosswalk - slowing down");
}
